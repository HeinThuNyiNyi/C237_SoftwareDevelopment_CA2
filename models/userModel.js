...
function getPublicStats(id, callback) {
    const sql = `
        SELECT
            ...
            verifiedRatings.reviewCount,
            verifiedRatings.averageRating,
            verifiedRatings.goodRatings
        FROM (
            SELECT
                COUNT(*) AS reviewCount,
                ROUND(AVG(r.rating), 1) AS averageRating,
                COALESCE(SUM(r.rating >= 4), 0) AS goodRatings
            FROM ratings r
            WHERE r.seller_id = ?
              AND EXISTS (
    ...
    `;
    db.query(sql, [id, id, id], callback);
}
